...
<body>
    <div class="receipt">
        <div class="header">
            ...
            <p>Rua Exemplo, 123 - Centro, Sao Paulo - SP</p>
        </div>
        ...
        <div class="info">
            ...
            @if($invoice->paid_date)<p><span class="label">Pago em:</span> {{ $invoice->paid_date->format('d/m/Y') }}</p>@endif
            <p><span class="label">Valor:</span> R$ {{ number_format($invoice->amount, 2, ',', '.') }}</p>
            @if($invoice->discount > 0)<p><span class="label">Desconto:</span> -R$ {{ number_format($invoice->discount, 2, ',', '.') }}</p>@endif
        </div>
        ...
        <div class="payment-details">
            ...
            <p>{{ $payment->payment_date->format('d/m/Y') }} - {{ strtoupper($payment->payment_method) }} - R$ {{ number_format($payment->amount, 2, ',', '.') }}</p>
            @endforeach
        </div>
    ...
    <div class="no-print">
        ...
        <br><br>
        <a href="{{ route('crm.portal.invoices.show', $invoice) }}" style="color: #2563eb; font-size: 13px;">Voltar para fatura</a>
    </div>
</body>
</html>
