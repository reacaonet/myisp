        ...
        <aside class="w-64 bg-gray-900 text-white flex flex-col shrink-0">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-1">
                ...
                <a href="{{ route('crm.portal.invoices') }}" class="flex items-center gap-3 px-3 py-2 rounded-lg {{ request()->routeIs('crm.portal.invoices*') ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-800' }}">
                    ...
                    Faturas
                    @php
                        $pendingCount = \Modules\Billing\Models\Invoice::where('client_id', Auth::guard('client')->id())->whereIn('status', ['pending', 'overdue'])->count();
                    @endphp
                    @if($pendingCount > 0)
                    <span class="ml-auto bg-red-500 text-white text-xs font-bold px-1.5 py-0.5 rounded-full">{{ $pendingCount }}</span>
                    @endif
                </a>
