...
<div class="max-w-3xl mx-auto space-y-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-6 border-b border-gray-200 flex items-center justify-between">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-1">Aberto em {{ $ticket->created_at->format('d/m/Y H:i') }}</p>
            </div>
        ...
        <div class="p-6">
            <dl class="grid grid-cols-2 gap-4 text-sm">
                ...
                <div>
                    ...
                    <dd class="font-medium text-gray-900">{{ $ticket->contract->plan->name }} ({{ $ticket->contract->codigo ?? '#' . $ticket->contract->id }})</dd>
                </div>
                @endif
            </dl>
        </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="p-6 space-y-4 max-h-96 overflow-y-auto">
            ...
            <div class="flex {{ $msg->sender_type === 'client' ? 'justify-end' : 'justify-start' }}">
                <div class="max-w-xs lg:max-w-md px-4 py-3 rounded-lg text-sm
                    {{ $msg->sender_type === 'client' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-800' }}">
                    <p class="text-xs font-medium mb-1 {{ $msg->sender_type === 'client' ? 'text-blue-200' : 'text-gray-500' }}">
                        {{ $msg->sender_type === 'client' ? 'Voce' : 'Suporte' }} &middot; {{ $msg->created_at->format('d/m/Y H:i') }}
                    </p>
        ...
        <div class="p-6 border-t border-gray-200">
            <form method="POST" action="{{ route('crm.portal.tickets.reply', $ticket) }}" class="space-y-3">
                @csrf
                <div>
                ...
                <div class="flex justify-end">
                    <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Enviar</button>
                </div>
            </form>
        </div>
        @else
        <div class="p-6 border-t border-gray-200 text-center text-gray-400 text-sm">
            Este chamado foi {{ $ticket->status === 'resolved' ? 'resolvido' : 'fechado' }}. Nao e possivel enviar novas mensagens.
        </div>
        @endif
    </div>

    <div class="text-center">
        <a href="{{ route('crm.portal.tickets') }}" class="text-sm text-blue-600 hover:underline">&larr; Voltar para chamados</a>
    </div>
</div>
@endsection
