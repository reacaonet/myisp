...
<div class="flex items-center justify-between mb-4 flex-wrap gap-3">
    <div class="flex items-center gap-3">
        <h1 class="text-xl font-bold text-gray-900">{{ $caixa->code }} - {{ $caixa->name }}</h1>
        ...
        @else
            <span class="px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-600">{{ ucfirst($caixa->status) }}</span>
        @endif
    </div>
    @if($caixa->status !== 'active')
    <form method="POST" action="{{ route('technician.portal.ftth.caixas.activate', $caixa) }}" onsubmit="return confirm('Ativar esta Caixa de Emenda? O escritorio vera a alteracao.');">
        @csrf
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
    <div class="lg:col-span-2 space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <h3 class="text-sm font-semibold text-gray-500 uppercase mb-4">Dados da Caixa de Emenda</h3>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                <div><p class="text-gray-400">Projeto</p><p class="font-medium text-gray-900">{{ $caixa->ftthProject?->name ?: '-' }}</p></div>
                <div><p class="text-gray-400">Porta da OLT</p><p class="font-mono font-medium text-gray-900">{{ $caixa->olt_port ?: '-' }}</p></div>
                <div><p class="text-gray-400">Splitter</p><p class="font-medium text-gray-900">{{ $caixa->splitter_config ?: '-' }}</p></div>
                <div><p class="text-gray-400">Capacidade</p><p class="font-medium text-gray-900">{{ $caixa->used_ports }}/{{ $caixa->capacity }}</p></div>
                <div class="col-span-2"><p class="text-gray-400">Endereco</p><p class="font-medium text-gray-900">{{ $caixa->full_address }}</p></div>
                <div><p class="text-gray-400">Latitude</p><p class="font-mono text-gray-900">{{ $caixa->latitude }}</p></div>
                <div><p class="text-gray-400">Longitude</p><p class="font-mono text-gray-900">{{ $caixa->longitude }}</p></div>
            </div>
    ...
    <div class="space-y-6">
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            ...
            @if($caixa->fusions_count > 0)
            <div class="flex items-center gap-4 mb-3">
                <span class="text-3xl font-bold text-green-600">{{ $doneCount }}</span>
                <span class="text-gray-300 text-xl">/</span>
                <span class="text-3xl font-bold text-gray-900">{{ $caixa->fusions_count }}</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-3 mb-2">
                <div class="bg-green-500 h-3 rounded-full transition-all" style="width: {{ $caixa->fusions_count > 0 ? round(($doneCount / $caixa->fusions_count) * 100) : 0 }}%"></div>
            </div>
            <p class="text-xs text-gray-400">{{ $caixa->fusions_count > 0 ? round(($doneCount / $caixa->fusions_count) * 100) : 0 }}% concluido</p>
            @if($pendingCount === 0)
        ...
        @if($caixa->ctos_count > 0)
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
            <h3 class="text-sm font-semibold text-gray-500 uppercase mb-3">CTOs Vinculadas</h3>
            <div class="space-y-2">
                @foreach($caixa->ctos as $cto)
                <a href="{{ route('technician.portal.ftth.ctos.show', $cto) }}" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition">
                    <div>
                    ...
                    <span class="px-2 py-0.5 rounded-full text-xs font-medium {{ $cto->status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">{{ ucfirst($cto->status) }}</span>
                </a>
                @endforeach
            </div>
        </div>
...
<div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    ...
    <form method="POST" action="{{ route('technician.portal.ftth.caixas.notes', $caixa) }}">
        @csrf
        <textarea name="technician_notes" rows="4" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500" placeholder="Ex: fibra 3 rompida, necessita reposicao...">{{ $caixa->technician_notes }}</textarea>
...
(function() {
    ...
    L.marker([{{ $caixa->latitude }}, {{ $caixa->longitude }}], { icon })
        .addTo(map)
        .bindPopup('<b>{{ $caixa->code }}</b><br>{{ $caixa->full_address }}')
        .openPopup();
