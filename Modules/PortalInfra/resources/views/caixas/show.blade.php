...
<div class="max-w-5xl mx-auto">
    <div class="flex items-center justify-between mb-4">
        <div class="flex items-center gap-3">
            <a href="{{ route('infra.ftth.caixas.index') }}" class="inline-flex items-center gap-1 px-3 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('infra.ftth.caixas.edit', $caixa) }}" class="inline-flex items-center gap-1 px-4 py-2 text-white rounded-lg text-sm font-medium" style="background-color:#2563eb;" onmouseover="this.style.backgroundColor='#1d4ed8'" onmouseout="this.style.backgroundColor='#2563eb'">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                Editar
            </a>
            <form method="POST" action="{{ route('infra.ftth.caixas.destroy', $caixa) }}" onsubmit="return confirm('Excluir esta Caixa de Emenda? Esta acao nao pode ser desfeita.');">
                @csrf @method('DELETE')
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        <div class="p-6 space-y-5">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
                <div>
                    <p class="text-xs text-gray-500 uppercase font-medium">CTOs Vinculadas</p>
                ...
                <div>
                    ...
                    <p class="text-sm font-mono text-gray-900">{{ $caixa->latitude }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase font-medium">Longitude</p>
                    <p class="text-sm font-mono text-gray-900">{{ $caixa->longitude }}</p>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-5">
                <div>
                    ...
                    <p class="text-sm text-gray-900">{{ $caixa->city ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase font-medium">Rua</p>
                    <p class="text-sm text-gray-900">{{ $caixa->street ?: '-' }}</p>
                </div>
                <div>
                    <p class="text-xs text-gray-500 uppercase font-medium">Status</p>
                    <p class="text-sm text-gray-900">{{ ucfirst($caixa->status) }}</p>
                </div>
            </div>
            ...
            <div class="border-t border-gray-100 pt-4">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-2">
                    ...
                    <a href="{{ route('infra.ftth.ctos.show', $cto) }}" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                        <div>
