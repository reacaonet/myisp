...
<div class="flex items-center justify-between mb-6">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('infra.ftth.export.csv.ctos', request()->query()) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-gray-100 text-gray-700 rounded-lg text-sm font-medium hover:bg-gray-200">
            ...
            <tr class="hover:bg-gray-50">
                ...
                <td class="px-4 py-3 text-sm">
                    @if($cto->caixa_emenda_id && $cto->caixaEmenda)
                        <a href="{{ route('infra.ftth.caixas.show', $cto->caixa_emenda_id) }}" class="text-purple-600 hover:underline">{{ $cto->caixaEmenda->code }}</a>
                    @else
                ...
                <td class="px-4 py-3 text-sm">
                    <div class="flex items-center gap-1">
                        <a href="{{ route('infra.ftth.ctos.show', $cto) }}" class="p-1.5 text-gray-400 hover:text-blue-600 rounded" title="Ver">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                        </a>
                        <a href="{{ route('infra.ftth.ctos.edit', $cto) }}" class="p-1.5 text-blue-500 hover:text-blue-700 rounded" title="Editar">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                        </a>
                        <form method="POST" action="{{ route('infra.ftth.ctos.destroy', $cto) }}" onsubmit="return confirm('Excluir esta CTO? Esta acao nao pode ser desfeita.')" class="inline">
                            @csrf @method('DELETE')
