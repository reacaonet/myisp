...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
        ...
        <a href="{{ route('infra.equipment.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Novo Equipamento</a>
    </div>
    ...
    <div class="p-4 border-b border-gray-200 bg-gray-50">
        <form method="GET" class="flex flex-wrap gap-3 items-end">
            ...
            @if(request()->hasAny(['search','type','status']))
                <a href="{{ route('infra.equipment.index') }}" class="px-4 py-2 text-sm font-medium text-gray-600 hover:text-gray-800">Limpar</a>
            @endif
        </form>
    </div>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50">
                    <td class="px-6 py-4 font-medium text-gray-900">
                        <a href="{{ route('infra.equipment.show', $e) }}" class="text-blue-600 hover:underline">{{ $e->name }}</a>
                    </td>
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-0.5">
                            <a href="{{ route('infra.equipment.show', $e) }}" title="Ver" class="p-1.5 rounded hover:bg-green-50 text-green-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg></a>
                            <a href="{{ route('infra.equipment.edit', $e) }}" title="Editar" class="p-1.5 rounded hover:bg-blue-50 text-blue-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                            <form method="POST" action="{{ route('infra.equipment.destroy', $e) }}" onsubmit="return confirm('Remover equipamento {{ $e->name }}?')" class="inline">
                                @csrf @method('DELETE')
