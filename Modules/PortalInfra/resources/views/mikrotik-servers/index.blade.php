...
<div class="bg-white rounded-xl shadow-sm border border-gray-200">
    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
        ...
        <a href="{{ route('infra.mikrotik-servers.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Novo Servidor</a>
    </div>
                ...
                <tr class="border-b border-gray-100 hover:bg-gray-50">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex items-center justify-end gap-0.5">
                            <form method="POST" action="{{ route('infra.mikrotik-servers.test', $s) }}" class="inline">
                                @csrf
                                <button type="submit" title="Testar" class="p-1.5 rounded hover:bg-green-50 text-green-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></button>
                            </form>
                            <a href="{{ route('infra.mikrotik-servers.edit', $s) }}" title="Editar" class="p-1.5 rounded hover:bg-blue-50 text-blue-600"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                            <form method="POST" action="{{ route('infra.mikrotik-servers.destroy', $s) }}" onsubmit="return confirm('Remover servidor {{ $s->name }}?')" class="inline">
                                @csrf @method('DELETE')
