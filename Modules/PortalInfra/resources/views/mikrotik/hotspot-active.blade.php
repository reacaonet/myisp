...
@section('title', 'Sessoes Ativas Hotspot')

@section('content')
<div class="max-w-6xl mx-auto">
    <div class="flex items-center justify-between mb-6">
        <h2 class="text-2xl font-bold text-gray-900">Sessoes Ativas Hotspot</h2>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
        <form method="GET" action="{{ route('infra.mikrotik.hotspot-active') }}">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <div class="p-4 border-b border-gray-200 flex items-center justify-between">
            <h3 class="font-semibold text-gray-900">
                {{ $selectedServer->name }} — {{ count($activeSessions) }} usuarios ativos
            </h3>
            <a href="{{ route('infra.mikrotik.hotspot-active', ['server_id' => $selectedServer->id]) }}"
            ...
            <table class="w-full text-sm">
                <thead class="bg-gray-50 border-b border-gray-200">
                    <tr>
                        ...
                        <th class="text-left px-4 py-3 font-medium text-gray-500">MAC</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3 font-medium">{{ $session['user'] ?? '' }}</td>
                        <td class="px-4 py-3 font-mono text-xs">{{ $session['address'] ?? '' }}</td>
                        <td class="px-4 py-3">{{ $session['login-by'] ?? '' }}</td>
                        <td class="px-4 py-3 font-mono text-xs">{{ $session['mac-address'] ?? '' }}</td>
                        <td class="px-4 py-3">{{ $session['server'] ?? '' }}</td>
                        <td class="px-4 py-3">
                            <form method="POST" action="{{ route('infra.mikrotik.kick-hotspot', $selectedServer->id) }}" class="inline"
                                  onsubmit="return confirm('Deseja desconectar este usuario?')">
                                @csrf
        ...
        @else
        <div class="p-8 text-center text-gray-500">Nenhum usuario Hotspot ativo neste servidor.</div>
        @endif
    </div>
    @endif
</div>
